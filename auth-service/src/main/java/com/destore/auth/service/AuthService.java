...
public class AuthService {
    ...
    public LoginResponse login(LoginRequest request) {
        ...
        User user = userRepository.findByUsername(request.getUsername())
                .orElseThrow(() -> new com.destore.exception.InvalidRequestException("Invalid username or password"));

        if (!user.getEnabled()) {
            throw new com.destore.exception.InvalidRequestException("User account is disabled");
        }

        if (!passwordEncoder.matches(request.getPassword(), user.getPasswordHash())) {
            throw new com.destore.exception.InvalidRequestException("Invalid username or password");
        }
    ...
    public User createUser(String username, String password, String email, String role) {
        if (userRepository.existsByUsername(username)) {
            throw new com.destore.exception.DuplicateResourceException("User", username);
        }
